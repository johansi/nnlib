...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=16, input_features=3):
        super(HourglassNet, self).__init__()

        self.inplanes = 64
        self.num_feats = 128
        self.num_stacks = num_stacks
        self.coordconv1 = CoordConv(input_features,self.inplanes,kernel_size=7,stride=2, padding=3,bias=True)
...
def hg(**kwargs):
    model = HourglassNet(Bottleneck, num_stacks=kwargs['num_stacks'], num_blocks=kwargs['num_blocks'],
                         num_classes=kwargs['num_classes'], input_features=kwargs['input_features'])
    return model
